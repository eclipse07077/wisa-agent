...
def main() -> None:
    ...
    with gzip.open(args.flow, "rt", encoding="utf-8") as handle:
        flow = json.load(handle)
    if hashlib.sha256(manifest_bytes).hexdigest() != flow[
    ...
    selections = {
        ...
        for mode, payload in flow["selections"].items()
    }
    result = {
        "method": flow["method"],
        ...
        "selections": {
            mode: {
                **selected_metrics(
                ...
                "objective": flow["selections"][mode]["objective"],
            }
        ...
        "candidate_count": flow["selections"]["full"]["candidates"],
        "chain_count": flow["chain_count"],
        "seeded_chain_count": flow["seeded_chain_count"],
        "runtime_seconds": flow["runtime_seconds"],
    }
